    ...
    if (-not $ShouldInstall) {
        ...
        if ($Workspace.TestInstall($TargetVersion)) {
            ...
            if ($Workspace.TestArchive($TargetVersion)) {
                ...
                Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation "Downloading Emacs version $TargetVersion..." -PercentComplete 57
                Write-CDInfo "Downloading Emacs version $TargetVersion..."

                $Archive = New-EmacsArchive $LatestDownload
            }

            Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation "Unpacking Emacs version $TargetVersion..." -PercentComplete 71
        ...
        Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Executing Administrator commands...' -PercentComplete 86
        Write-CDInfo "Executing Administrator commands..."

        $ModulePath = (Get-Module -ListAvailable Cackledaemon).Path
        ...
        Invoke-AsAdministrator {
            ...
            Write-Output "updating emacs path"
            Update-EmacsInstall -Path $Install
            Write-Output "setting emacs app path registry keys"
            Set-EmacsAppPathRegistryKeys
            Write-Output "installing file type associations"
