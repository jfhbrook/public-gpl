...
function Wait-ForEmacsProcessExit {
    [CmdletBinding()]
    param(
        [int]$PollingInterval = 1  # 1 Second
    )

    $OpenProcesses = Get-OpenEmacsProcesses

    while ($OpenProcesses) {
        Write-LogInformation "The following Emacs processes are open:"

        $OpenProcesses | Write-LogInformation

        Write-LogInformation "Please close these processes to continue."
    ...
    if (-not $ShouldInstall) {
        ...
        if ($Workspace.TestInstall($TargetVersion)) {
            ...
            if ($Workspace.TestArchive($TargetVersion)) {
                Write-LogVerbose "Emacs has already been downloaded (but not unpacked) for version $TargetVersion"
...
task . Clean, DownloadDependencies, Build, Test

#+END_SRC
** Downloading Vendored Dependencies
In order to make Cackledaemon easier to install while also modularizing things a
little bit, we download some (a) vendored dependency.
#+BEGIN_SRC powershell :tangle Cackledaemon.Build.ps1
task DownloadDependencies {
    Invoke-WebRequest `
      'https://raw.githubusercontent.com/jfhbrook/PSeudo/master/PSeudo/PSeudo.psm1' `
      -OutFile '.\Cackledaemon\Pseudo.psm1'
}
#+END_SRC
