...
function New-CDLogRecord {
    ...
    Write-Information -MessageData $MessageData -Tags (@($Level) + $Tags) 6>&1
}
...
function Enable-LogRotateJob {
    ...
    Enable-Job 'LogRotateJob' {
        ...
        while ($True) {
            Invoke-LogRotate
            Write-CDDebug ('LogRotateJob sleeping for {0} seconds.' -f $LogCheckTime)
...
function Install-FileTypeAssociations {
    Get-FileTypeAssociationsConfig | ForEach-Object {
        Write-Host ('{0}={1}' -f $_.Extension, $_.FileType)
        Write-Host  ('{0}={1}' -f $_.FileType, $_.Command)
...
For this, we use [[https://github.com/jfhbrook/PSeudo][PSeudo]], a module that I maintain for this purpose. Development
is ongoing and I'm currently going through some gnar licensing issues, but in
the meantime these are some notes from a prior version:

The [[https://knowyourmeme.com/memes/galaxy-brain][tiny brain]] version of this is to simply run ~Start-Process~ with the ~-Verb
RunAs~ flag turned on and the desired code to be executed inside of a string.
This works and is relatively safe. However, any IO that happens in that process
stays with that process, and if the window is hidden or the process surprise
exits then any information not written to a log file is lost.
...
When on the happy path, this means that you can pass a string or script block
(which will implicitly get stringified correctly) to this function and it will
run the code in an Administrator shell, seamlessly dumping the output into the
parent terminal, "just like sudo".

However, it's not very sophisticated. It only covers ~Write-Host~ and
~Write-Error~ for output, leaving ~Debug~, ~Verbose~ and ~Information~ out in
the cold. It also does very little to handle rich data structures or even
multi-line strings. Code that gets passed to this function needs to be
constructed carefully so as to cause the expected output behavior. If this were
going to be turned into a separate module, more care would need to be taken to
ensure complete coverage of IO functions.

It's also important to note that named pipes in Windows are shockingly brittle.
To wit: The ~WaitForConnection~ method is blocking, is not ~C-c~-able, and will
stop the parent PowerShell process from exiting.

TL;DR: This procedure evaluates string inputs at elevated privilege levels with
the added ability of being able to brick the host process, so it's imperative
that any script authors ensure that strings passed to this function are sound
and not vulnerable to injections or error explosions.
** Ensuring that Emacs is Closed
The Windows OS will absolutely refuse to move or delete a file that is currently
open in another process, and that includes running executables, such as
~emacs.exe~ and ~emacsclientw.exe~. This means that if we want Cackledeamon's
update process to be user-friendly that it has to guide the user through this
process instead of hard crashing.
...
function Wait-ForEmacsProcessExit {
    ...
    while ($OpenProcesses) {
        if (((Get-Date) - $StartTime).TotalSeconds > $Timeout) {
...
function Install-Emacs {
    ...
    if (Test-Workspace) {
        ...
        Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Creating workspace...' -PercentComplete 10
        Write-CDInfo 'Creating new workspace...'

        $Workspace = New-Workspace
    }

    Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Checking the Emacs website for the latest available download...' -PercentComplete 20
    ...
    Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Checking if Emacs needs to be installed or updated...' -PercentComplete 30

    if ($Force) {
        Write-CDInfo "`$Force flag was enabled so installing Emacs regardless of what's installed"
    ...
    if (-not $ShouldInstall) {
        ...
        if ($Workspace.TestInstall($TargetVersion)) {
            ...
            if ($Workspace.TestArchive($TargetVersion)) {
                ...
                Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation "Downloading Emacs version $TargetVersion..." -PercentComplete 40
                Write-CDInfo "Downloading Emacs version $TargetVersion..."

                $Archive = New-EmacsArchive $LatestDownload
            }

            Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation "Unpacking Emacs version $TargetVersion..." -PercentComplete 50
        ...
        Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Elevating privileges...' -PercentComplete 60
        ...
        Invoke-AsAdministrator "
            ...
            Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Updating Emacs...' -PercentComplete 70
            Write-CDInfo 'Updating Emacs...'
            Update-EmacsInstall -Path $Install | Out-Null

            Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Setting app path registry keys...' -PercentComplete 80
            Write-CDInfo 'Setting app path registry keys...'
            Set-EmacsAppPathRegistryKeys | Out-Null

            Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Installing file type associations...' -PercentComplete 90
            Write-CDInfo 'Installing file type associations...'
