...
Emacs Client,emacsclientw.exe,"[]",GNU EmacsClient: The client for the extensible self-documenting text editor
#+END_SRC
...
Describe 'Install-Shortcuts' {
    ...
    It 'installs shortcuts' {
        ...
        'TestDrive:\AppData\Microsoft\Windows\Start Menu\Programs\Gnu Emacs\Emacs Client.lnk' | Should -Exist
    }
}
...
Describe 'Start-EmacsDaemon' {
    ...
    Context 'when Emacs is not running and -Wait is false' {
        It 'starts Emacs in the background' {
            ...
            Mock -Module Cackledaemon Get-UnmanagedEmacsDaemons { $null }
            Mock -Module Cackledaemon Start-Process { $TestProcess }
            Mock -Module Cackledaemon Write-ProcessToPidFile { $null }
            Mock -Module Cackledaemon Wait-Process { $TestProcess }
            ...
            Assert-MockCalled -Module Cackledaemon Start-Process -Times 1 -ParameterFilter {
                $RedirectStandardOut -eq "$TestDrive\Cackledaemon\EmacsStdOut.log" -and $RedirectStandardError -eq "$TestDrive\Cackledaemon\EmacsStdError.log"
            }
            Assert-MockCalled -Module Cackledaemon Write-ProcessToPidFile -Times 1
            Assert-MockCalled -Module Cackledaemon Write-ProcessToPidFile -Times 1 -ParameterFilter {
                $Process -eq $TestProcess
            }
    ...
    Context 'when Emacs is not running and -Wait is true' {
        It 'starts Emacs and then waits' {
            Mock -Module Cackledaemon Get-ProcessFromPidFile { $null }
            Mock -Module Cackledaemon Get-UnmanagedEmacsDaemons { $null }
            Mock -Module Cackledaemon Start-Process { $null }
            Mock -Module Cackledaemon Write-ProcessToPidFile { $null }
            Mock -Module Cackledaemon Wait-Process { $TestProcess }
