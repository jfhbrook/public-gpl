...
function Install-Emacs {
    ...
    if (Test-EmacsExe) {
        ...
        Write-Progress -Activity 'Checking the currently installed Emacs version' -CurrentOperation 'Comparing versions...' -PercentComplete 67
    ...
    Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation "Updating the user's `$Path variable..." -PercentComplete 70
    Write-LogHost "Updating the user's `$Path variable..."

    Set-EmacsPathEnvVariable

    Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation "Setting the user's `$HOME variable..." -PercentComplete 80

    Write-LogHost "Setting the user's `$HOME variable..."
    Set-HomeEnvVariable

    Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation "Installing shortcuts..." -PercentComplete 90
