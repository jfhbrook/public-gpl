...
#+END_SRC
* Installation
#+BEGIN_SRC powershell :tangle ./playground.ps1
...
$EmacsWindowsEndpoint = 'https://ftp.gnu.org/gnu/emacs/windows/'
...
function Get-AvailableEmacsDownloads {
    (Invoke-WebRequest $EmacsWindowsEndpoint).Links | ForEach-Object {
        if ($_.href -match '^emacs-(\d+)/$') {
            $MajorPathPart = $_.href
            (Invoke-WebRequest ($EmacsWindowsEndpoint + $_.href)).Links | ForEach-Object {
                 if ($_.href -match '^emacs-(\d+)\.(\d+)-x86_64\.zip$') {
                     $Href = $EmacsWindowsEndpoint + $MajorPathPart + $_.href
...
function Get-LatestEmacsDownload {
    (Get-AvailableEmacsDownloads | Measure-Object -Maximum).Maximum
}
...
class Workspace : IDisposable {
    ...
    Workspace([string]$Path) {
        ...
        if (Test-Path $Path) {
            ...
            $this.Root = New-Item -Type Directory $Path
        }
        if (Test-Path $ArchivesPath) {
            $this.Archives = Get-Item $ArchivesPath
        } else {
            $this.Archives = New-Item -Type Directory $ArchivesPath
        }
        if (Test-Path $InstallsPath) {
            $this.Installs = Get-Item $InstallsPath
        } else {
            $this.Installs = New-Item -Type Directory $InstallsPath
        }
        if (Test-Path $BackupsPath) {
            $this.Backups = Get-Item $BackupsPath
        } else {
            $this.Backups = New-Item -Type Directory $BackupsPath
        }
    }

    Dispose() {
        Remove-Item $this.Root -Recurse
    }
}
...
function New-TempWorkspace {
    $TempDir = Join-Path `
      ...
      ('cackledaemon-{0}' -f (New-GUID))

    return New-Object Workspace $TempDir
}
...
function Update-EmacsInstall {
    ...
    $Destination = $EmacsInstallLocation
    ...
    Move-Item $Source $Destination -ErrorAction Stop
    Remove-Item $Backup
