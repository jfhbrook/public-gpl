...
function Initialize-TestEnvironment {
    ...
    $Global:CackledaemonConfigLocation = Join-Path $CackledaemonWD 'Configuration.ps1'
...
Describe 'Invoke-LogRotate' {
    It 'rotates the logs' {
        ...
        'TestDrive:\Cackledaemon\Cackledaemon.log.3' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.3' | Should -FileContentMatch 'Testing 1'

        Write-LogVerbose 'Testing 4'
        Invoke-LogRotate

        'TestDrive:\Cackledaemon\Cackledaemon.log' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log' | Should -Not -FileContentMatch 'Testing 4'
        'TestDrive:\Cackledaemon\Cackledaemon.log.1' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.1' | Should -FileContentMatch 'Testing 4'
        'TestDrive:\Cackledaemon\Cackledaemon.log.2' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.2' | Should -FileContentMatch 'Testing 3'
        'TestDrive:\Cackledaemon\Cackledaemon.log.3' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.3' | Should -FileContentMatch 'Testing 2'
        'TestDrive:\Cackledaemon\Cackledaemon.log.4' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.4' | Should -FileContentMatch 'Testing 1'

        Write-LogVerbose 'Testing 5'
        Invoke-LogRotate

        'TestDrive:\Cackledaemon\Cackledaemon.log' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log' | Should -Not -FileContentMatch 'Testing 5'
        'TestDrive:\Cackledaemon\Cackledaemon.log.1' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.1' | Should -FileContentMatch 'Testing 5'
        'TestDrive:\Cackledaemon\Cackledaemon.log.2' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.2' | Should -FileContentMatch 'Testing 4'
        'TestDrive:\Cackledaemon\Cackledaemon.log.3' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.3' | Should -FileContentMatch 'Testing 3'
        'TestDrive:\Cackledaemon\Cackledaemon.log.4' | Should -Exist
        'TestDrive:\Cackledaemon\Cackledaemon.log.4' | Should -FileContentMatch 'Testing 2'
        'TestDrive:\Cackledaemon\Cackledaemon.log.5' | Should -Not -Exist
...
function Test-EmacsExe {
    ...
    Test-Item (Join-Path $EmacsInstallLocation 'bin' 'emacs.exe')
}
...
function Get-EmacsExeVersion {
    if (Get-Command 'emacs.exe' -ErrorAction SilentlyContinue) {
        if ((emacs.exe --version)[0] -match '^GNU Emacs (\d+)\.(\d+)$') {
            New-Object Version $Matches[1], $Matches[2]
        }
    }
}
...
Note that Download isn't comparable because there isn't a use case for comparing
downloads with other downloads, and while the IComparable interface is game to
compare disparate objects, PowerShell is not. Therefore, we always compare the
Version properties.
...
function Get-EmacsDownload {
    ...
    Write-LogInfo ('Checking {0} for available Emacs versions...' -f $EmacsDownloadsEndpoint)
    (Invoke-WebRequest $EmacDownloadsEndpoint).Links | ForEach-Object {
        if ($_.href -match '^emacs-(\d+)/$') {
            $MajorPathPart = $_.href
            (Invoke-WebRequest ($EmacDownloadsEndpoint + $_.href)).Links | ForEach-Object {
                 if ($_.href -match '^emacs-(\d+)\.(\d+)-x86_64\.zip$') {
                     $Href = $EmacsDownloadsEndpoint + $MajorPathPart + $_.href
                     return New-Object Download $Matches[1], $Matches[2], $Href
                 }
            }
        }
    } | Where-Object {$_}
...
class Workspace {
    ...
    GetKey([Version]$Version) {
        'emacs-{0}.{1}-x86_64.zip' -f $Version.Major, $Version.Minor
    }

    GetArchivePath([Version]$Version) {
        Join-Path $this.Archives $this.GetKey($Version)
    }

    TestArchive([Version]$Version) {
        Test-Item $this.GetArchivePath($Version)
    }

    GetArchive([Version]$Version) {
        Get-Item $this.GetArchivePath($Version)
    }

    GetInstallPath([Version]$Version) {
        Join-Path $this.Installs $this.GetKey($Version)
    }

    TestInstall([Version]$Version) {
        Test-Item $this.GetInstallPath($Version)
    }

    GetInstall([Version]$Version) {
        Get-Item $this.GetInstallPath($Version)
    }
...
function New-Workspace {
    ...
    New-Item -Type Directory $ArchivesPath
    New-Item -Type Directory $InstallsPath
    New-Item -Type Directory $BackupsPath
...
function New-EmacsArchive {
    ...
    $Archive = Join-Path $Workspace.Archives.FullName (Split-Path $Download.Href -Leaf)
