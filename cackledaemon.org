...
function Invoke-AsAdministrator {
    ...
    # We give the process a chance to not immediately hard crash, in the hopes
    # that we don't cause a lock by waiting for a connection we never receive
    ...
    while ($True) {
        ...
        while ($Line) {
            if ($Line.StartsWith('HOST:')) {
                $Line -replace '^HOST\:', '' | Write-Host
            } elseif ($Line.StartsWith('ERROR:')) {
                $Line -replace '^ERROR\:', '' | Write-Error
            } elseif ($Line.StartsWith('EXIT:')) {
                $Line -replace '^EXIT\:', '' | ForEach-Object {
...
function Install-Emacs {
    ...
    if (-not $ShouldInstall) {
        ...
        Invoke-AsAdministrator {
            ...
            Write-Host 'Moving files to $EmacsInstallLocation...'
            Update-EmacsInstall -Path $Install

            Write-Host 'Setting App Path registry keys...'
            Set-EmacsAppPathRegistryKeys
