...
#+BEGIN_SRC powershell :tangle ./Cackledaemon/InstallWizard.ps1 :noweb yes
<<license-header>>
...
Starting the Emacs daemon is a matter of calling [[https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/start-process?view=powershell-5.1][Start-Process]]. Emacs is
configured to run in the background and redirect stdio to log files in
Cackledaemon's working directory.
...
the PID file and attempting to stop the process with [[https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.management/stop-process?view=powershell-5.1][Stop-Process]].
...
whic is to use the associated application to open the item. In the case of
...
After all of the menu items ad callbacks are configured, the icon is started by
