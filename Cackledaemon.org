#+TITLE: Cackledaemon - An Emacs Installation and Daemon Manager for Windows
...
to create when setting up the user's environment and which file type
associations to create when installing Emacs. These settings are stored in
...
Desired file type associations are also stored in a CSV file. The format of the
command string is detailed in [[https://docs.microsoft.com/en-us/windows-server/administration/windows-commands/ftype][the documentation for the ~ftype~ batch command.]]
...
Note that the ~TYPE~ header is necessary, as Cackledeamon uses the same
technique to load the CSV as with shortcuts, albeit without ~CovertFrom-Json~.
...
Testing is also similar to what's done for shortcuts.
...
function Install-Emacs {
    ...
    if (-not $ShouldInstall) {
        ...
        Invoke-AsAdministrator "
            ...
            Write-Progress -Id 1 -Activity 'Installing Emacs' -CurrentOperation 'Installing file type associations...' -PercentComplete 81
